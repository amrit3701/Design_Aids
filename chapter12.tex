...
\tablemacro \ref{chaptertable} gives for $p = \sage{pp.n(digits=2)}$, and steel \Fefouronefivemacro,
...
\equmacro \ref{eq:depth-cover} gives with $c = \sage{cc.n(digits=2)} cm,\phi=\sage{pphi.n(digits=2)}$,
...
It may be noted that with $D$ = $\sage{s11}$  $cm$, this type of footing of uniform depth is   not economical.
